...
tabItem(tabName = "accueil",
	...
	Ils s'attaquent respectivement aux insectes ravageurs, aux champignons, aux « mauvaises herbes » et aux vers parasites.", a("(Wikipédia)", href="https://fr.wikipedia.org/wiki/Pesticide", target="_blank")),
	...
	fluidRow(
		box(title = actionLink("link_to_tabpanel_station", "Quelle est la qualité de l'eau chez moi? Chez mes proches?"),
			...
			column(width=4,
				...
				actionButton("geoloc_accueil", "Voir ma ville", class="btn btn-primary center", style="width:90% ; color: white; font-weight: bold; font-size:20px")
			),
			column(width=8,
	...
	fluidRow(
		...
		box(title = actionLink("link_to_tabpanel_danger", "Les pesticides sont-ils dangereux?"),
			width = 6, solidHeader = TRUE, collapsible = FALSE, background = NULL, status = "danger",
			helpText("Tout ce que vous avez toujours voulu savoir sur les pesticides !"),
			valueBox(1043, "pesticides répertoriés", icon = icon("book"), color="red"),
...
tabItem(tabName = "classification",
	...
	fluidRow(
		box(width=8,
			...
			helpText("Zommer sur une famille pour voir les molécules qui la composent. Cliquez dans la barre du haut pour revenir à l'état initial."),
		...
		column(width=4,
			conditionalPanel(id="details_treemap", condition = "input.treemapchoice == 'nombre de pesticides'",
				infoBox( width=12,
					a("2013 interdiction UE", href="http://www.lemonde.fr/planete/article/2013/05/24/abeilles-interdiction-de-trois-pesticides-dans-l-ue-a-compter-du-1er-decembre_3416897_3244.html", target="_blank"),
					"thiaméthoxam, clothianidine, imidacloprid", icon = icon("remove"), color ="red"
				),
				infoBox( width=12,
					a("2003 interdiction UE", href="https://www.senat.fr/rap/l02-215-2/l02-215-241.html", target="_blank"),
					"atrazine", icon = icon("remove"), color ="red"
				),
				infoBox( width=12,
					"2003 interdiction UE","oxadixyl, fongicide (mildiou)", icon = icon("remove"),  color ="red"
				)
				#actionLink("link_to_tabpanel_station", "Voir sur la carte", class="btn btn-primary", style="width:100% ; color: white; font-weight: bold") 
				#onclick="link_to_tabpanel_evolution")		
			),
			conditionalPanel(id="details_treemap", condition = "input.treemapchoice == 'quantité mesurée'",
				infoBox( width=12,
					a("2003 interdiction UE", href="https://www.senat.fr/rap/l02-215-2/l02-215-241.html", target="_blank"),
				...
				box(
				  title="atrazine", width= 12, solidHeader = TRUE, status = "danger", 
				  "2e herbicide le plus utilisé aux USA. Suspectée d’être une des causes du phénomène récent qui décime les amphibiens sur toute la planète."
				),
				box(
				  title="thiamétoxam", width= 12, solidHeader = TRUE, status = "warning",
				  "La substance active du Cruiser intoxique les abeilles même à faible dose.", a("source", href="http://www.actu-environnement.com/ae/news/ministere-agriculture-cruiser-osr-etude-inra-acta-cruiser-thiamethoxam-abeilles-15340.php4", target="_blank")
				),
				box(
					title="piperonyl butoxide", width= 12, solidHeader = TRUE, status = "primary",
					"Un synergisant, un produit chimique ajouté aux pesticides pour accroître la toxicité des ingrédients actifs"
				)
			)
		)
	)
	...
	fluidRow(id="toxicite_row",
		tabBox(title=NULL, width=12,
			tabPanel("Lesquels sont les plus toxiques?",
				fluidRow(
					column(width=6, offset=3, align="justify", 
						"Ce graphique décrit la toxicité des pesticides retrouvés en france: de très toxique (en haut),
						à une toxicité modérée (en bas). La 
						taille des bulles est proportionnelle 
						au nombre de fois où la molécule a été détecté en 2012. Vous pouvez zoomer sur une zone (cliquer-glisser) et faire 
						apparaitre / disparaitre les familles en cliquant sur la légende."
						)),
				br(),
				fluidRow(align="center",
					plotlyOutput("bubblechart", width="800px")
					),
				...
				fluidRow(
					...
					column(width=5,
						box(width=4,align="center",
							...
							p(align="justify", "Le DDT a un impact for sur l'épaisseur des coquilles d'oeuf. La réduction de l'épaisseur de celles ci peut aller jusqu'à 90%!", a("(source)", href="http://www.iewonline.be/IMG/pdf/4_pesticides-substancesactives.pdf", target="_blank"))					
						)
					)
				)
			)
...
tabItem("apropos",
	br(),
	fluidRow(column(offset=3, 6, box(align="justify", width=12, solidHeader = FALSE, status="info", title=h1("Le GreenTech Challenge"),
		"Le ",
		...
		". L'objectif est de d'améliorer la connaissance du grand publique en ce qui concerne les pesticides.",
		"Pour ce faire, un important ",
		a("jeux de données", href="http://www.donnees.statistiques.developpement-durable.gouv.fr/dataviz_pesticides/", target="blank()"),
		" a été mis à la disposition des candidat, comprenant plus de 2 millions de relevés dans plus de 2000 stations de mesures"
		))),
		
	
	fluidRow(
	column(offset=1, 5, box(align="justify", width=12, solidHeader = FALSE, status="info", title=h1("Team Superviz"),
		...
		". Amis de longue date et tous geek dans l'âme, il ne nous a pas fallu plus de 5 minutes pour se décider à se lancer dans cette aventure après avoir (tardivement) entendu parlé de ce challenge.",
		"Vous pouvez en apprendre plus sur nos personnalités via les pages web suivantes:" ,
		br(),br(),
		"Charles Moszkowicz:", a("company",href="http://eneo.fr/fr/contact/", target="_blank"),br(),   
	...
	column(offset=0, 5, box(align="justify", width=12, solidHeader = FALSE, status="info", title=h1("L'application"),
		...
		br(),br(),
		"En espérant que vous y trouverez toutes les informations que vous recherchez, l'équipe Superviz vous souhaite une bonne naviguation!"
		)))	
)
